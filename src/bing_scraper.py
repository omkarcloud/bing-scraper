...
def print_data_errors(credits_exhausted, not_subscribed, unknown_error, no_key):
    ...
    if credits_exhausted:
        ...
        print(f"Could not get data for {len(credits_exhausted)} {name} due to credit exhaustion. Please consider upgrading your plan by visiting https://rapidapi.com/Chetan11dev/api/bing-scraper/pricing to continue scraping data.")

    if not_subscribed:
        name = "queries" if len(not_subscribed) > 1 else "query"
        print(f"Could not get data for {len(not_subscribed)} {name} as you are not subscribed to Bing Scraper API. Please subscribe to a free plan by visiting https://rapidapi.com/Chetan11dev/api/bing-scraper/pricing")
    ...
    if no_key:
        ...
        print(f"Could not get data for {len(no_key)} {name} as you are not subscribed to Bing Scraper API. Please subscribe to a free plan by visiting https://rapidapi.com/Chetan11dev/api/bing-scraper/pricing")
...
class Bing:
    ...
    def search(query:  Union[str, List[str]],  max: Optional[int] = None, key: Optional[str] =None,  use_cache: bool = True) -> Dict:
        """
        Function to scrape data from Bing.
